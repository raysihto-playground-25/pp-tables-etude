...
ストーリーカードは以下の厳格な固定フォーマットに従って記述する：

```
【xxx】は、【yyy】したい。【zzz】(の) ために。
```

各要素の定義は以下の通り：

- **【xxx】**: ユーザーロール (Who) - 後述の定義済みリストから選択
- **【yyy】**: 実現したいこと (What) - 行いたい行動や操作
- **【zzz】**: 理由・価値 (Why) - なぜそれを行いたいのか

詳細な定義と考察は付録 C を参照。

#### 1.4.3 定義済みユーザーロール

ストーリーカードの【xxx】として使用可能なユーザーロールは以下の通り：
...
レイアウトモックアップは、SVG形式で提供しています。GitHubのファイルブラウザー上で直接確認できます。
...
- **投票結果表示エリア**: 
  ...
  - 代表値をテーブル中央に表示
  - 各カードの色は乖離度を表現
- **投票パネル**: ドロップダウン (単一選択select) でポイント選択
- 操作ボタン: Reveal、Reset
...
- **モバイル版**: 
  ...
  - **投票タブ**: 
    ...
    - 投票パネルはドロップダウン (単一選択select) でポイント選択
    - 参加者簡易状況表示は廃止
  - ユーザータブで詳細な参加者管理
  - チャットタブで会話履歴を表示
- **投票結果表示**: 
...
- **サーバーレス (Serverless) アーキテクチャが理想**
  - サーバープログラムコードの実装を極力避ける
  - マネージドサービスとIaC (Infrastructure as Code) による定義のみでコントロール
  - プログラミングは必要最小限に留める方針

**具体的な選択肢:**
- **言語**: Node.js (TypeScript)、Python、またはGo
- **フレームワーク**: Express.js、FastAPI、またはGin (サーバープログラムが必要な場合)
- **サーバーレスオプション**: AWS Lambda、Google Cloud Functions、Azure Functions
- **リアルタイム通信**: AWS AppSync (GraphQL subscriptions)、Firebase Realtime Database、またはWebSocketライブラリ
...
- **ストーリーカード**: ユーザーストーリーを記述するための記法・情報。「【xxx】は、【yyy】したい。【zzz】(の) ために。」という固定フォーマットで記述される。単体では管理せず、ユーザーストーリー/PBI の一部として扱う
- **ユーザーロール**: ストーリーカードにおける【xxx】の要素。ストーリーの主語となるユーザーの立場や役割を表す。定義済みリストから選択する
...
本節では、プロダクトバックログ管理における用語の階層関係、ストーリーカードの詳細仕様、およびフォーマットの設計根拠を補足的に説明する。

### ストーリーカードの詳細仕様

本節では、セクション 1.4 で定義したストーリーカードの詳細な仕様と運用ガイドラインを記述する。
...
例として、「初回ユーザー」「移動中のユーザー」「障害対応中の管理者」などは、それぞれ異なる期待や切迫感を持つため、ロールとして扱う理由がある。
...
```
【エンドユーザー (開発者)】は、投票結果を一覧表示したい。チーム全体の見積もり傾向を把握するために。
【プロダクトオーナー】は、各ラウンドの履歴を確認したい。過去の見積もり精度を分析・改善するために。
【システム管理者】は、非アクティブなテーブルを自動削除したい。サーバーリソースを効率的に管理するために。
【モバイルユーザー】は、外出先から投票に参加したい。オフィスにいなくても見積もりに貢献するために。
【開発チーム (本システムの)】は、クラッシュレポートを自動収集したい。バグの早期発見と修正を効率化するために。
```
...
本ガイドラインは、ユーザーストーリーにおけるユーザーロール (xxx) を通じて、以下をチーム内で瞬時に共有できる状態を作ることを目的とする：
...
- 本仕様書の定義済みユーザーロールリストに追加し、仕様変更として扱う
...
**本仕様書における取り扱い:**
- 本仕様書の主たる関心事は「ユーザーストーリー」と「ストーリーカード」の定義である
- バグ修正、スパイク、タスクなどの詳細な管理方法は、プロジェクト進行に応じて別途定義する

### ストーリーカードフォーマットと 5W2H の関係性

本節では、本仕様書で定義したストーリーカードフォーマットと、情報整理の基本フレームワークである 5W2H との関係性について考察する。
...
1. **【xxx】= Who (誰が)**
   - ユーザーロールとして明示的に記述
   - 定義済みリストから選択することで一貫性を確保
   - 例: 「エンドユーザー (開発者)」「モバイルユーザー」
...
- **When (いつ)**: ストーリーカードでは記述しない。実装の優先順位やスプリント計画で別途管理する
- **Where (どこで)**: 必要に応じて xxx (Who) で表現 (例: 「モバイルユーザー」は暗黙的に「外出先で」を含む)
...
**代替案 1: 英語表記の使用**
```
【Who】は、【What】したい。【Why】(の) ために。
```
- メリット: 5W2H との対応が明確
- デメリット: 日本語の自然な文章構造が損なわれる、学習コストが上がる
...
```
- メリット: 意味が直感的に理解しやすい
- デメリット: 「誰がは」という二重助詞のような不自然な表現になる

**代替案 3: 現行の xxx/yyy/zzz 表記 (採用)**
```
【xxx】は、【yyy】したい。【zzz】(の) ために。
```
- メリット:
  - 日本語として自然な文章構造を維持
  - xxx/yyy/zzz は抽象的な変数として認識され、柔軟性が高い
  - 具体的な意味 (Who/What/Why) は説明文で明確に定義
  - 読み手が内容に集中できる (表記自体が意味を主張しすぎない)
- デメリット:
  - 初見では xxx/yyy/zzz の意味が不明瞭 (ただし説明を読めば解決)

**考察結果と結論:**

本仕様書では、以下の理由により現行の xxx/yyy/zzz 表記を採用する：

1. **自然な日本語**: 「【xxx】は、【yyy】したい。【zzz】のために。」という文章構造は日本語として自然であり、読みやすい

2. **抽象性の利点**: xxx/yyy/zzz という変数的な表記は、記述者が「何を書くべきか」よりも「どのような内容を書くべきか」に集中できる

3. **フォーカスの明確化**: ストーリーカードは Who/What/Why の 3 要素に絞ることで、ユーザー価値に集中できる。When/Where/How/How much は別の管理手段で扱う

4. **一貫性**: 定義済みユーザーロールリストと組み合わせることで、xxx の記述揺れを防ぎ、一貫性を確保できる

5. **5W2H との関係性の明確化**: 本付録で 5W2H との対応を明示することで、フレームワークの理解を補完できる

したがって、現行フォーマット `【xxx】は、【yyy】したい。【zzz】(の) ために。` を維持し、xxx = Who、yyy = What、zzz = Why という対応関係を本仕様書で明確に文書化することが最適であると結論づける。
