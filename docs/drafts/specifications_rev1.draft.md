...
- 後述の定義済みユーザーロールリストから選択する (新規ロールの追加は仕様変更として扱う)
...
- 現時点では、プロダクトバックログは本仕様書 (specifications_rev1.draft.md) 以外の別の場所で列挙・管理することを想定している
- 具体的な管理場所やツール (GitHub Issues、専用のバックログ管理ツールなど) は未定であり、プロジェクトの進行に応じて決定する
...
本節では、プロダクトバックログ管理における用語の階層関係を補足的に説明する。
