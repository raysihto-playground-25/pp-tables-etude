...
- 投票が {3, 8} の場合: 平均 = 5.5 → フィボナッチ数5と8の距離を比較: |5-5.5|=0.5 == |8-5.5|=0.5 → 等距離のため大きい方の 8 を選択
- 投票が {3, 3, 3, 8, 8, 8} の場合: 平均 = 5.5 → フィボナッチ数5と8の距離を比較: |5-5.5|=0.5 == |8-5.5|=0.5 → 等距離のため大きい方の 8 を選択

**注意:** 上記の例では、フィボナッチ数列全体から最も近い値を探す。投票された値だけでなく、0, 1, 2, 3, 5, 8, 13, 21, 34 などのフィボナッチ数全体が候補となる。
...
*例1: {3, 3, 3, 8, 8, 8}*
- 平均 = (3×3 + 8×3) / 6 = (9 + 24) / 6 = 33/6 = 5.5
- フィボナッチ数5と8の距離: |5-5.5|=0.5, |8-5.5|=0.5 → 等距離のため大きい方の **代表値は 8**
- 3: 乖離 -5（大乖離）→ 赤系
- 8: 乖離 0（乖離ゼロ）→ 色強調なし
- **結論**: 8が代表値に一致するため色なし、3は大きく乖離するため赤系

*例2: {3, 3, 3, 5, 8, 8, 8}*
- 平均 = (3×3 + 5 + 8×3) / 7 = (9 + 5 + 24) / 7 = 38/7 ≈ 5.43
- フィボナッチ数5の距離: |5-5.43|=0.43
- フィボナッチ数8の距離: |8-5.43|=2.57
- **代表値は 5**（最も近い）
- 3: 乖離 -2（小乖離）→ 薄い黄色
- 5: 乖離 0（乖離ゼロ）→ 色強調なし
- 8: 乖離 +3（中乖離）→ オレンジ系
