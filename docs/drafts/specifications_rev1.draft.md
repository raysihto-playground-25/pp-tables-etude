...
**計算例:**
- {3, 5, 8}: 平均 = 5.33 → |5-5.33|=0.33 < |8-5.33|=2.67 → 代表値は 5
- {3, 8, 13}: 平均 = 8.0 → |8-8.0|=0 → 代表値は 8
- {2, 5, 8}: 平均 = 5.0 → |5-5.0|=0 → 代表値は 5
- {3, 8}: 平均 = 5.5 → |5-5.5|=0.5 < |8-5.5|=2.5 → 代表値は 5
- {3, 3, 3, 8, 8, 8}: 平均 = 5.5 → |5-5.5|=0.5 < |8-5.5|=2.5 → 代表値は 5
...
**乖離の判定:**
- すべての投票者の乖離値が 0 の場合 (全員が代表値から等距離) は「乖離なし」として、すべてのカードに色強調を行わない
- それ以外の場合は、乖離値に応じて色を付ける

**色分けルール:**
- **乖離 0 (最も代表値に近いグループ)**: 色強調なし (デフォルト背景色)
- **乖離 1-2**: 薄い黄色系
- **乖離 3-5**: オレンジ系
- **乖離 6以上**: 赤系
...
- 代表値: 5
- 各投票の距離: 2→|2-5|=3, 5→|5-5|=0, 8→|8-5|=3
- 最小距離: 0
- 乖離値: 2→3, 5→0, 8→3
- **判定**:
  - 5: 乖離 0 → 色強調なし
...
- 代表値: 8 (|8-7.25|=0.75 < |5-7.25|=2.25)
- 各投票の距離: 3→|3-8|=5, 5→|5-8|=3, 8→|8-8|=0, 13→|13-8|=5
- 最小距離: 0
- 乖離値: 3→5, 5→3, 8→0, 13→5
- **判定**:
  - 8: 乖離 0 → 色強調なし
  - 5: 乖離 3 → オレンジ系
  - 3: 乖離 5 → オレンジ系
