...
2. 算術平均に最も近いフィボナッチ数を代表値とする
3. 近さが同じ場合（等距離の場合）は、大きい方の値を選択

**例:**
- 投票が {3, 5, 8} の場合: 平均 = 5.33 → 代表値は 5
- 投票が {3, 8, 13} の場合: 平均 = 8.0 → 代表値は 8
- 投票が {2, 5, 8} の場合: 平均 = 5.0 → 代表値は 5
- 投票が {3, 8} の場合: 平均 = 5.5 → 5と8が等距離（差0.5）、大きい方の8を選択
...
*例1: {3, 3, 3, 8, 8, 8}*
- 平均 = 5.5 → 代表値は 8（5と8が等距離、大きい方）
- 全員が代表値から等距離（3は-5、8は0だが、最小距離は0、最大距離は5で全員同じではない）
- 実際には: 3が-5の乖離、8が0の乖離
- **修正**: すべての人が代表値から「等しく遠い」とは、全員の乖離の絶対値が同じ場合を指す
- この例では、3のグループ（乖離-5）と8のグループ（乖離0）で異なるため、通常の色分けを適用
- **再解釈**: 「全員が等しく遠い」= 全投票値が代表値から同じ距離
- {3,3,3,8,8,8}の場合、代表値5.5→8を選択すると、3は5離れ、8は0離れで異なる
- 代表値5を選択すると、3は2離れ、8は3離れで異なる
- **結論**: この例では全員が等距離ではないため、通常通り色付け

*例2（全員等距離）: {3, 3, 3, 8, 8, 8}を再検討*
- ユーザーの意図を確認: 平均5.5で、3から5.5までの距離は2.5、8から5.5までの距離は2.5
- **全員が等距離！** → すべてのカードに色強調なし
