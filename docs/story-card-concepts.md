...
- 動詞形式で具体的に表現
- 例: 「投票結果を一覧表示したい」
- **How (どのように)** や **How much (どのくらい)** の要素を含む場合がある (例: 「素早く検索したい」「大量のデータを一括処理したい」)
...
- **How much (どのくらい)** の要素が理由・価値として表れる場合がある (例: 「短時間で結果を得るために」)
...
- **How (どのように)**: ユーザーが何かを行う具体的な方法・手段。{action} と部分的に重なるが、より詳細な手順やプロセスは受け入れ条件 (Acceptance Criteria) や仕様として別途定義する
- **How much (いくら / どのくらい)**: ユーザーが望む量・程度・規模。例えば「どのくらいの速さで」「どれだけの量を」などの要件は、{action} や {benefit} に含めるか、非機能要件・受け入れ条件として別途定義する
