...
本ドキュメントは、ユーザーストーリー駆動開発に関する一般的・汎用的な概念を定義する規範的標準である。ストーリーカード、完成の定義 (DoD: Definition of Done)、受け入れ条件 (AC: Acceptance Criteria) など、ユーザーストーリーを中心とした開発プロセスの中核要素を網羅する。これらは本プロジェクトを支える確定事項として、プロジェクト固有の仕様とは独立して管理される。

---

## Status

本ドキュメントは、本プロジェクトにおいて**規範的 (normative)** である。

MUST と表記される項目は必須要件である。SHOULD と表記される項目は、正当な理由がある場合を除き、強く推奨される。MAY と表記される項目は任意の実践である。

MUST 要件からの逸脱には、明示的な承認と文書化が必要である。

---

## Requirement Levels

本ドキュメントでは、[RFC 2119](https://www.rfc-editor.org/rfc/rfc2119) および [RFC 8174](https://www.rfc-editor.org/rfc/rfc8174) で定義される要件レベルのキーワードを使用する。

以下のキーワードが用いられる：

- **MUST**: 絶対的な要件
- **MUST NOT**: 絶対的な禁止
- **SHOULD**: 推奨されるベストプラクティス
- **MAY**: 任意の実践

**重要**: これらのキーワードは、すべて大文字で表記される場合に限り、RFC 2119 および RFC 8174 で定義される特別な意味を持つ。小文字または大文字小文字混在で表記される場合は、通常の日本語としての意味で解釈される。
...
本標準は以下に適用される：
- 本プロジェクト内で PBI として管理されるすべてのユーザーストーリー
- 設計、開発、レビューに参加するすべての貢献者
...
### Part I – Normative Standards (規範的標準)
...
6. [PBI (Product Backlog Item) の概念](#pbi-product-backlog-item-の概念)
7. [Compliance Verification (適合性検証)](#compliance-verification-適合性検証)

### Part II – Recommended Practices (推奨プラクティス)

8. [ユーザーロール運用ガイドライン](#ユーザーロール運用ガイドライン)
9. [DoD の運用と段階的な進化](#dod-の運用と段階的な進化)
10. [5W2H フレームワークとの関係性](#5w2h-フレームワークとの関係性)
...
- ストーリー横断の共通完成基準を定義する
- すべてのユーザーストーリーに適用される絶対的な条件

**ユーザーストーリー (PBI)**
```
ユーザーストーリー (PBI)
  ├── ストーリーカード: 「誰が・何を・なぜ」を表現
  └── 受け入れ条件 (AC): 「個々のストーリー固有の受け入れ可能条件」を定義
```

**重要な関係性:**
- 各ユーザーストーリーは上記 DoD を満たさなければならない (MUST satisfy DoD)
- AC (受け入れ条件) は DoD を満たすことを助けるストーリー固有の条件であり、DoD の必須要素の一つとして位置づけられる
- DoD には「すべての受け入れ条件 (AC) が満たされること」が含まれなければならない (MUST include AC fulfillment)

#### 1. ストーリーカード (Story Card)

- **目的**: ユーザー視点での価値を表現する
- **形式**: `{role} は、 {action} したい。 {benefit} (の) ために。`
- **役割**: 開発の目的と方向性を示す
...
- **重要**: DoD の必須条件の一つとして「すべての受け入れ条件 (AC) が満たされること」が含まれなければならない (MUST include AC fulfillment)
...
**重要な関係性:**
- ストーリーが「完成」とみなされるには、**DoD (完成の定義) を満たさなければならない (MUST satisfy DoD)**
- DoD の必須条件の一つとして、**「すべての受け入れ条件 (AC) が満たされていること」が含まれなければならない (MUST include AC fulfillment)**
- つまり、DoD は絶対的な条件であり、AC は DoD の一要素として位置づけられる
...
ストーリーカードフォーマットは以下であることが推奨される (SHOULD be):

```
{role} は、 {action} したい。 {benefit} (の) ために。
```

各プレースホルダーと半角スペース、句読点の位置は固定であることが推奨される (SHOULD be fixed)。
...
- 定義済みユーザーロールリストから選択しなければならない (MUST select from predefined list)。新規ロールの追加は仕様変更として扱う
- カードの記述や開発時の揺れやブレを防ぐため、自由記述は許可されない (MUST NOT use free-form text)
- 人・役職・権限名に限定されず、必要に応じて **when / where / condition (状況・文脈)** を含めてよい (MAY include)
...
- すべてのストーリーに一律に適用されなければならない (MUST apply to all stories)
- 個別ストーリーごとに定義される受け入れ条件 (AC) の具体的内容は別途管理される
- DoD 側には「当該ストーリーの AC をすべて満たしていること」という共通要件が含まれなければならない (MUST include AC fulfillment)
- 品質の一貫性を保証する

### DoD の必須構造

すべての DoD は以下の階層構造を持たなければならない (MUST have the following hierarchy):

```
User Story Completion (ユーザーストーリーの完成)
└── MUST satisfy DoD (DoD を満たさなければならない)
      └── MUST satisfy all AC (すべての AC を満たさなければならない)
         その他の品質基準・プロセス基準はプロジェクト固有
```

**重要な構造的要件:**
- AC の充足だけでは不十分である (AC fulfillment alone is insufficient)
- DoD には AC 以外にも、プロジェクト固有の品質基準やプロセス基準が含まれる
- DoD のすべての条件を満たして初めて「完成」とみなされる
...
受け入れ条件 (AC: Acceptance Criteria) は、**個々のストーリー固有の機能要件や受け入れ可能条件を定義する**ものである。

**基本原則:**
- ストーリーカードが「なぜ・何を」を示すのに対し、AC は「どのような条件を満たせば受け入れ可能か」を明確にする
- 各ユーザーストーリーに固有の条件を記述する
- 検証可能で明確な条件として定義しなければならない (MUST be verifiable and clear)
- 開発者とビジネス側が合意する受け入れテストの基準となる
- ストーリーの完成判定そのものは DoD (その必須要素として AC 充足を含む) で行う
...
```
受け入れ条件:
- 投票権限: Voter ロールの参加者のみ投票できる
- 投票タイミング: ラウンド開始後、公開前まで投票可能
- 投票変更: 公開前であれば投票を変更可能
- 公開条件: すべての Voter が投票完了、または運営者が強制公開
...
| 記述者 | ビジネス側 + 開発チーム | チーム全体で合意 |
| 変更頻度 | ストーリーごと | プロジェクト単位 |
| 位置づけ | DoD の必須要素の一つ | 絶対的な完成基準 |
| 例 | "投票後に結果が表示される" | "すべての AC が満たされている" / "ユニットテストが追加されている" |

**重要な関係性:**
- **DoD (完成の定義)** は、ストーリーが「完成」とみなされるための絶対的な条件である (MUST be satisfied)
- **AC (受け入れ条件)** は、DoD の必須条件の一つとして位置づけられる (MUST be included in DoD)
- 「すべての受け入れ条件 (AC) が満たされていること」は、DoD の必須項目として含まれなければならない (MUST be included in DoD)
- したがって、AC を満たさなければ DoD を満たすことはできず、ストーリーは「未完成」となる
...
## Compliance Verification (適合性検証)

### 完成判定前の検証要件

ストーリーを完成と判定する前に、以下の検証が行われなければならない (MUST be verified):

1. **DoD チェックリストの検証**
   - DoD のすべての項目が満たされていることを確認しなければならない (MUST verify all DoD items)
   - 各項目について明示的な確認を行う

2. **AC チェックリストの検証**
   - すべての受け入れ条件 (AC) が満たされていることを確認しなければならない (MUST verify all AC)
   - 各 AC について検証可能な証拠を確認する

3. **レビュアーによる適合性確認**
   - レビュアーは上記の DoD および AC の検証を確認しなければならない (MUST confirm compliance)
   - ストーリーを完成とする承認前に適合性を検証する

### 検証の責任

- 開発者: DoD および AC の充足に責任を持つ
- レビュアー: 完成判定前に DoD および AC の検証を確認する責任を持つ
- チーム: DoD の定義と更新に責任を持つ
...
本セクションは推奨プラクティスであり、参考情報として提供される。

### DoD の運用

#### 1. チーム全体で合意

- DoD はチーム全員が参加して定義することが推奨される (SHOULD be defined by entire team)
- 開発者、QA、デザイナーなど、すべての役割が納得する内容にすることが推奨される (SHOULD be agreed by all roles)
- 定期的に見直し、プロジェクトの成熟度に応じて更新することが推奨される (SHOULD be reviewed regularly)

#### 2. 現実的な基準設定

- 達成不可能な基準は避けるべきである (SHOULD avoid unachievable criteria)
- チームのスキルレベルやプロジェクトのフェーズに応じて調整する
- 初期は基本的な DoD から始め、段階的に厳格化することが推奨される (SHOULD start simple and evolve)

#### 3. 継続的な改善

- 定期的な振り返りで DoD を見直すことが推奨される (SHOULD review regularly)
- 問題が繰り返し発生する場合は、DoD に項目を追加する
- 不要になった項目は削除または調整する

#### 4. 可視化と共有

- DoD をチーム全員が見える場所に掲示することが推奨される (SHOULD make visible)
- 新メンバーには必ず DoD を説明する
- ストーリーの完成判定時には必ず DoD を参照する

### DoD の段階的な進化

プロジェクトの成熟度に応じて、DoD を進化させる：

#### フェーズ1: 最小限の DoD (プロジェクト初期)
...
本ドキュメントは、ユーザーストーリー駆動開発に関する一般的・汎用的な概念を定義している。ストーリーカード、完成の定義 (DoD)、受け入れ条件 (AC) などの基本概念と運用ガイドラインを提供し、プロジェクト全体の開発標準として機能する。プロジェクト固有のユーザーロール定義、優先順位付けルール、具体的な DoD の内容は、各プロジェクトの仕様書で定義される。
