# User Story-Driven Development Standards & Practices
...
MUST 要件からの逸脱は、明示的な承認と文書化を必要とする。
...
本標準は以下に適用される：
- Project X 内で PBI として管理されるすべてのユーザーストーリー
...
### Part I – 規範的標準 (Normative Standards)
...
7. [コンプライアンス検証 (Compliance Verification)](#コンプライアンス検証-compliance-verification)

### Part II – 推奨プラクティス (Recommended Practices)
8. [ユーザーロール運用ガイドライン](#ユーザーロール運用ガイドライン)
9. [5W2H フレームワークとの関係性](#5w2h-フレームワークとの関係性)

---

# Part I – 規範的標準 (Normative Standards)

本セクションに含まれるすべての要件は拘束力を持つ。
...
- DoD には「すべての受け入れ条件 (AC) が満たされること」が含まれなければならない (MUST include)
...
- **重要**: DoD の必須条件の一つとして「すべての受け入れ条件 (AC) が満たされること」が含まれる
...
- DoD の必須条件の一つとして、**「すべての受け入れ条件 (AC) が満たされていること」が含まれなければならない (MUST include)**
...
ストーリーカードのフォーマットは以下でなければならない (MUST be)：

```
{role} は、 {action} したい。 {benefit} (の) ために。
```

各プレースホルダーと半角スペース、句読点の位置は固定である (MUST be fixed)。
...
- 定義済みユーザーロールリストから選択しなければならない (MUST select) (新規ロールの追加は仕様変更として扱う)
- カードの記述や開発時の揺れやブレを防ぐため、自由記述は許可してはならない (MUST NOT permit)
- 人・役職・権限名に限定されず、必要に応じて **when / where / condition (状況・文脈)** を含めてもよい (MAY include)
...
- DoD 側には「当該ストーリーの AC をすべて満たしていること」という共通要件が含まれる
- 品質の一貫性を保証する
...
- DoD はチーム全員が参加して定義すべきである (SHOULD define)
- 開発者、QA、デザイナーなど、すべての役割が納得する内容にすべきである (SHOULD ensure)
- 定期的に見直し、プロジェクトの成熟度に応じて更新すべきである (SHOULD review)

#### 2. 現実的な基準設定

- 達成不可能な基準は避けるべきである (SHOULD avoid)
- チームのスキルレベルやプロジェクトのフェーズに応じて調整すべきである (SHOULD adjust)
- 初期は基本的な DoD から始め、段階的に厳格化すべきである (SHOULD start)

#### 3. 継続的な改善

- 定期的な振り返りで DoD を見直すべきである (SHOULD review)
- 問題が繰り返し発生する場合は、DoD に項目を追加すべきである (SHOULD add)
- 不要になった項目は削除または調整すべきである (SHOULD remove)

#### 4. 可視化と共有

- DoD をチーム全員が見える場所に掲示すべきである (SHOULD display)
- 新メンバーには必ず DoD を説明すべきである (SHOULD explain)
- ストーリーの完成判定時には必ず DoD を参照しなければならない (MUST reference)
...
DoD は**ストーリー横断の絶対的な完成基準**であり、AC は DoD の必須要素の一つである。

#### 完成基準の階層構造

```
ユーザーストーリーの完成 (User Story Completion)
└── DoD を満たさなければならない (MUST satisfy DoD)
      ├── すべての AC を満たさなければならない (MUST satisfy all AC)
      ├── 必須テストに合格しなければならない (MUST pass required tests)
      └── プロセス基準を満たさなければならない (MUST meet process criteria)
```

**重要:** AC の充足のみでは不十分である。ストーリーを完成とみなすには、DoD のすべての条件を満たさなければならない (MUST satisfy all DoD conditions)。
...
- **AC (受け入れ条件)** は、DoD の必須条件の一つとして位置づけられる
- ストーリーが完成するには、「すべての AC が満たされていること」を含む、DoD のすべての条件を満たさなければならない (MUST satisfy)
- AC を満たしても、DoD の他の条件 (テスト、レビュー等) を満たしていなければ「未完成」である
...
- 各ユーザーストーリーに固有の条件を記述しなければならない (MUST describe)
- 検証可能で明確な条件として定義しなければならない (MUST define)
- 開発者とビジネス側が合意する受け入れテストの基準となる
- ストーリーの完成判定そのものは DoD (その必須要素として AC 充足を含む) で行わなければならない (MUST be performed)
...
- **DoD (完成の定義)** は、ストーリーが「完成」とみなされるための絶対的な条件である
- **AC (受け入れ条件)** は、DoD の必須条件の一つとして位置づけられる
- 「すべての受け入れ条件 (AC) が満たされていること」は、DoD の必須項目として含まれなければならない (MUST include)
...
## コンプライアンス検証 (Compliance Verification)

ストーリーを完成とマークする前に、以下を実施しなければならない (MUST)：

### 検証チェックリスト

- [ ] DoD チェックリストが検証されていなければならない (MUST be verified)
- [ ] AC チェックリストが検証されていなければならない (MUST be verified)
- [ ] レビュアーがコンプライアンスを確認しなければならない (MUST confirm compliance)

### 検証プロセス

1. **開発者の責任**
   - すべての AC が満たされていることを確認する
   - DoD の各項目をチェックする
   - レビュー前にすべての要件が満たされていることを保証する

2. **レビュアーの責任**
   - AC の充足を独立して検証する
   - DoD の各項目が満たされていることを確認する
   - コンプライアンス違反がある場合は承認してはならない (MUST NOT approve)
...
これらの条件がすべて満たされた場合のみ、ストーリーを「完成」とマークしてよい (MAY mark as complete)。

---

# Part II – 推奨プラクティス (Recommended Practices)

本セクションに含まれるガイドラインは拘束力を持たないが、強く推奨される。
...
- ユーザーロールは事前定義リストから選択すべきである (SHOULD select)
- 新規ロールを追加する場合は、上記判断基準に照らして検討すべきである (SHOULD review)
- プロジェクトの仕様書の定義済みユーザーロールリストに追加し、仕様変更として扱うべきである (SHOULD treat as specification change)
- 迷った場合は、「この違いで、開発の判断は本当に変わるか？」に立ち戻るべきである (SHOULD return to)
- ユーザーストーリーは定期的に見直し、優先順位の再評価を行うべきである (SHOULD review)
- エンドユーザーの価値を最大化することを常に念頭に置き、バックログの優先順位付けを行うべきである (SHOULD prioritize)
