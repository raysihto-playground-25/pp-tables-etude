...
ストーリーカードフォーマットは以下でなければならない (MUST be):

```
{role} は、 {action} したい。 {benefit} (の) ために。
```

各プレースホルダーと半角スペース、句読点の位置は固定でなければならない (MUST be fixed)。
...
└── MUST satisfy DoD (DoD を満たさなければならない)
      ...
      ├── MUST pass required tests (必須テストに合格しなければならない)
      └── MUST meet process criteria (プロセス基準を満たさなければならない)
```

**重要な構造的要件:**
- AC の充足だけでは不十分である (AC fulfillment alone is insufficient)
- DoD のすべての条件を満たして初めて「完成」とみなされる
- AC は DoD の必須要素であるが、DoD はそれ以上の品質基準を含む
...
ストーリーが完成とマークされる前に、以下の検証が行われなければならない (MUST be verified):
