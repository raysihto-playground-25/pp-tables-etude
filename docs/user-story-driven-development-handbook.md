...
本ドキュメントは、ユーザーストーリー駆動開発に関する一般的・汎用的な概念を定義する。ストーリーカード、受け入れ条件 (AC: Acceptance Criteria)、完了の定義 (DoD: Definition of Done) など、ユーザーストーリーを中心とした開発プロセスの中核要素を網羅する。これらは本プロジェクトを支える確定事項として、プロジェクト固有の仕様とは独立して管理される。
...
2. [ストーリーカードとユーザーストーリーの関係](#ストーリーカードとユーザーストーリーの関係)
3. [ストーリーカードのフォーマット要素](#ストーリーカードのフォーマット要素)
4. [受け入れ条件 (AC: Acceptance Criteria)](#受け入れ条件-ac-acceptance-criteria)
5. [完了の定義 (DoD: Definition of Done)](#完了の定義-dod-definition-of-done)
...
ユーザーストーリー駆動開発は、**ユーザー価値を中心に据えた開発手法**である。ユーザーストーリーを起点として、要求定義、開発、検証、完了判定までを一貫して管理する。
...
ユーザーストーリー (PBI)
  ...
  ├── 受け入れ条件 (AC): 「どのような条件を満たせば完成か」を定義
  └── 完了の定義 (DoD): 「完了とみなすための品質基準」を定義
```
...
#### 2. 受け入れ条件 (AC: Acceptance Criteria)

- **目的**: ストーリーが完成したと判断する具体的な条件を定義する
- **形式**: 検証可能なチェックリストや Given-When-Then シナリオ
- **役割**: 開発スコープの明確化と受け入れテストの基準

#### 3. 完了の定義 (DoD: Definition of Done)

- **目的**: すべてのストーリーに共通する品質基準を定義する
- **形式**: プロジェクト全体で合意された完了条件のリスト
- **役割**: 品質の一貫性確保と「完了」の共通認識
...
- 検証可能で明確な条件として定義する
- 開発者とプロダクトオーナーが合意する受け入れテストの基準となる
...
- プロダクトオーナーが受け入れる際の判断基準となる
- 受け入れテストのテストケースの元となる
- 自動テストの要件定義としても活用できる

#### 3. コミュニケーションツール

- 開発者、プロダクトオーナー、QAが共通認識を持つための基盤
- 曖昧さを排除し、議論の土台を提供する
...
| 観点 | AC (受け入れ条件) | DoD (完了の定義) |
|------|------------------|------------------|
| スコープ | 個別ストーリー固有 | プロジェクト全体共通 |
| 内容 | 機能要件・振る舞い | 品質基準・プロセス |
| 記述者 | PO + 開発チーム | チーム全体で合意 |
| 変更頻度 | ストーリーごと | プロジェクト単位 |
| 例 | "投票後に結果が表示される" | "ユニットテストが追加されている" |

**重要:** AC と DoD は排他的ではなく、補完的な関係にある。ストーリーが「完了」とみなされるには、**AC と DoD の両方を満たす必要がある**。
...
完了の定義 (DoD: Definition of Done) は、**すべてのユーザーストーリーに共通して適用される品質基準**である。

**基本原則:**
- プロジェクトまたはチーム全体で合意された「完了」の基準
- すべてのストーリーに一律に適用される
...
- 「完了」の意味がステークホルダーに明確に伝わる
- プロダクトオーナーが安心して受け入れられる
- スプリントレビューでの混乱を防ぐ
...
- DoDはチーム全員が参加して定義する
- プロダクトオーナー、開発者、QA、デザイナーなど、すべての役割が納得する内容にする
...
```
ユーザーストーリーの「完了」判定
  ├── AC (受け入れ条件) を満たす → 機能が完成している
  └── DoD (完了の定義) を満たす → 品質基準を満たしている
```

**重要:** ストーリーがDone (完了) とみなされるには、**個別のAC + 共通のDoD** の両方を満たす必要がある。ACを満たしても、DoDを満たしていなければ「未完了」である。
...
- **When (いつ)** / **Where (どこで)**: {role} の一部として表現するか、実装の優先順位やスプリント計画で別途管理する
...
本ドキュメントは、ユーザーストーリー駆動開発に関する一般的・汎用的な概念を定義している。ストーリーカード、受け入れ条件 (AC)、完了の定義 (DoD) などの基本概念と運用ガイドラインを提供し、プロジェクト全体の開発標準として機能する。プロジェクト固有のユーザーロール定義、優先順位付けルール、具体的なDoDの内容は、各プロジェクトの仕様書で定義される。
